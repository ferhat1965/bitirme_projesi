# Gerçek Zamanlı Yol Kusuru Tespit ve Haritalama Sistemi: Teknik Rapor

Bu rapor, geliştirilen bitirme projesinin teknik altyapısını, kullanılan teknolojileri, sistem mimarisini ve çalışma prensiplerini detaylandırmak amacıyla hazırlanmıştır.

---

## 1. Proje Özeti
Bu çalışma, sürücü güvenliğini artırmak ve yol bakım süreçlerini otomatize etmek amacıyla geliştirilmiş mobil tabanlı bir yapay zeka çözümüdür. Sistem, araç kamerasından gelen canlı görüntüyü analiz ederek yoldaki çukurları gerçek zamanlı olarak tespit eder, GPS konumlarıyla eşleştirir ve merkezi/yerel bir veritabanına kaydeder.
...
Sistem, hibrit bir mimari kullanarak hem cihaz üzerinde (edge computing) hem de bulutta çalışabilmektedir.
...
4.  **Koordinat Dönüşümü:** Tespit edilen sınırlayıcı kutu (Bounding Box) koordinatları, ekran çözünürlüğüne uygun hale getirilir.
5.  **Veri Kaydı:** Tespit edilen çukurun; fotoğrafı, doğruluk oranı, tarih/saat bilgisi ve GPS koordinatları yerel SQLite veritabanına ve eşzamanlı olarak backend sunucusuna kaydedilir.

---

## 4. Teknik Detay: C++ ve JNI (Java Native Interface) İlişkisi

Mobil cihazlarda yapay zeka modellerini yüksek performansla çalıştırmak için Dart veya Java gibi yüksek seviyeli diller doğrudan işlemciye (CPU) veya grafik işlemciye (GPU) erişimde yavaş kalabilir. Projedeki bu darboğaz şu şekilde aşılmıştır:
...
*   **Performans:** Görüntü matrisleri C++ katmanında bellek adresleri üzerinden işlenir. Bu sayede, görüntü işleme süreçleri milisaniyeler seviyesine iner ve uygulamada donma olmadan gerçek zamanlı tespit yapılabilir.
...
2.  **Sunucu Üzerinde (Remote):** `records.db` (SQLite/FastAPI)
    *   Tüm cihazlardan gelen verilerin toplandığı merkezi havuz.

---

## 6. Sonuç
Proje; Flutter'ın esnek UI yeteneklerini, YOLOv8'in güçlü nesne tespitiyle ve C++/JNI'nın yüksek performansı ile birleştirerek, uçtan uca çalışan profesyonel bir mühendislik çözümüdür. Sunulan mimari, düşük gecikme süresi ve yüksek doğruluk oranı ile gerçek dünya koşullarında çalışmaya uygundur.
